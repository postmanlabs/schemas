    ...
    describe('package.json', function () {
        ...
        describe('package.json JSON data', function () {
            it('should have valid name, description and author', function () {
                expect(json).to.have.property('name', 'postman-schemas');
                expect(json).to.have.property('description',
                    'Tools and data related to Postman\'s JSON-based file formats');
                expect(json).to.have.property('author', 'Postman Labs <help@getpostman.com> (=)');
                expect(json).to.have.property('license', 'Apache-2.0');
                expect(json).to.have.property('homepage', 'https://github.com/postmanlabs/schemas');
                expect(json).to.have.property('bugs', 'https://github.com/postmanlabs/schemas/issues');

                expect(json).to.have.property('repository');
                expect(json.repository).to.eql({
                    type: 'git',
                    url: 'git://github.com/postmanlabs/schemas.git'
                });

                expect(json).to.have.property('keywords');
                expect(json.keywords).to.eql(['postman', 'collection', 'json-schema']);
